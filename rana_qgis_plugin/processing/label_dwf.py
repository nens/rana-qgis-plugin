...
def add_substance_concentration(input_file, output_file, substance_id):
    ...
    for entry in data:
        ...
        entry["substances"] = [
            {"substance": substance_id, "concentrations": concentrations}
        ]
...
def label_dwf(
    ...
    api_client = get_threedi_api()

    simulation_template = api_client.simulation_templates_read(simulation_template_id)
    feedback.pushInfo(
        f"Found simulation template '{simulation_template.name}' with id {simulation_template.id}"
    )

    original_simulation_id = simulation_template.simulation.id
    lateral_files = api_client.simulations_events_lateral_file_list(
        original_simulation_id
    ).results
    dwf_files = [f for f in lateral_files if f.periodic == "daily"]
    if not dwf_files:
        feedback.pushInfo(
            f"Simulation template does not have any dry weather flow events"
        )
        return
    ...
    dwf_substance = api_client.simulations_substances_create(
        simulation.id, {"name": "DWF (label)", "units": "%"}
    )
    feedback.pushInfo(
        f"Created substance '{dwf_substance.name}' with id {dwf_substance.id}"
    )
    ...
    for original_dwf_file in original_dwf_files:
        if original_dwf_file.periodic == "daily":
            original_dwf_file_download_url = (
                api_client.simulations_events_lateral_file_download(
                    simulation_pk=simulation_template.simulation.id,
    ...
    for original_dwf_file_path in original_dwf_file_paths:
        ...
        add_substance_concentration(
            original_dwf_file_path, edited_dwf_file_path, dwf_substance.id
        )
        edited_dwf_file_paths.append(edited_dwf_file_path)
        feedback.pushInfo(
            f"Added a label to {edited_dwf_file_path}, using substance id {dwf_substance.id}"
        )
    ...
    for lateral_file in lateral_files:
        if lateral_file.periodic == "daily":
            api_client.simulations_events_lateral_file_delete(
                lateral_file.id, simulation.id
            )
    ...
    for edited_dwf_file_path in edited_dwf_file_paths:
        ...
        laterals = api_client.simulations_events_lateral_file_list(
        ...
        for i in range(max_retries):
            state = api_client.simulations_events_lateral_file_read(
                simulation_pk=simulation.id, id=lateral.id
            ).file.state
            if state not in ["created", "uploaded", "processed"]:
                raise Exception(
                    f"Something went wrong while processing uploaded laterals. State: {state}"
                )
            if state == "processed":
                break
    ...
    feedback.pushInfo(
        f"Added simulation '{simulation.name}' with id {simulation.id} to queue"
    )
