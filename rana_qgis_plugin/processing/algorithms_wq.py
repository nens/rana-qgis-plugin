...
from rana_qgis_plugin.processing.label_dwf import label_dwf
...
class SimulateWithRainZonesAlgorithm(QgsProcessingAlgorithm):
    ...
    def name(self):
        return "rana_simulate_with_rain_zones"
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterString(self.SIMULATION_NAME, "Simulation name")
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        """
        simulation_template_id = self.parameterAsInt(
            parameters, self.SIMULATION_TEMPLATE_ID, context
        )
        simulation_name = self.parameterAsString(
            parameters, self.SIMULATION_NAME, context
        )
        polygon_feature_source = self.parameterAsSource(
            parameters, self.POLYGON_LAYER, context
        )
        polygon_feature_layer = self.parameterAsVectorLayer(
            parameters, self.POLYGON_LAYER, context
        )
        polygon_name_field = self.parameterAsString(
...
class SimulateWithDWFLabellingAlgorithm(QgsProcessingAlgorithm):
    ...
    def name(self):
        return "rana_simulate_with_dwf_labelling"
