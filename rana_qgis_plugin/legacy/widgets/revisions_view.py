...
from rana_qgis_plugin.legacy.widgets.utils_file_action import FileAction
            ...
            for i, revision in enumerate(revisions):
                ...
                if revision.has_threedimodel:
                    model_btn_data = BTNData(
                        "Delete",
                        lambda _, rev_id=revision.id: (
                            self.delete_3di_model_clicked.emit(rev_id)
                        ),
                        True,
                        None,
                    )
                    ...
                    model_btn_data = BTNData(
                        "Create",
                        lambda _, rev_id=revision.id: (
                            self.create_3di_model_clicked.emit(rev_id)
                        ),
                        create_enabled,
                        create_tooltip,
                    )
