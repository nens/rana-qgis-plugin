...
class WSProgressesSentinel(QObject):
    ...
    def fetch_finished_simulations(self):
        ...
        try:
            ...
            if self.model_id:
                logger.debug(
                    f"Filtering simulation statuses on model id {self.model_id}"
                )
            ...
            logger.debug(
                f"Fetched {len(finished_simulations_data)} finished simulation statuses"
            )
            time.sleep(1)
...
class UploadProgressWorker(QRunnable):
    ...
    def delete_sqlite_task(self):
        ...
        self.current_task_progress = 0
        self.report_upload_progress()
        self.tc.delete_schematisation_revision_sqlite(
            self.schematisation.id, self.revision.id
        )
        self.current_task_progress = 100
        self.report_upload_progress()
    ...
    def upload_raster_task(self, raster_type):
        ...
        raster_revision = self.tc.create_schematisation_revision_raster(
            ...
            self.revision.id,
            raster_file,
            raster_type=SchematisationRasterReferences.api_client_raster_type(
                raster_type
            ),
        )
    ...
    def delete_raster_task(self, raster_type):
        ...
        types_to_delete = [
            SchematisationRasterReferences.api_client_raster_type(raster_type)
        ]
    ...
    def commit_revision_task(self):
        ...
        for i in range(self.UPLOAD_CHECK_RETRIES):
            before_commit_revision = self.tc.fetch_schematisation_revision(
                self.schematisation.id, self.revision.id
            )
        ...
        self.tc.commit_schematisation_revision(
            self.schematisation.id, self.revision.id, commit_message=commit_message
        )
        self.revision = self.tc.fetch_schematisation_revision(
            self.schematisation.id, self.revision.id
        )
    ...
    def create_3di_model_task(self, inherit_templates=False):
        ...
        revision_tasks = self.tc.fetch_schematisation_revision_tasks(
            self.schematisation.id, self.revision.id
        )
